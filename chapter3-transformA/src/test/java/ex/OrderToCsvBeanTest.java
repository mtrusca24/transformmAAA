...
public class OrderToCsvBeanTest extends CamelTestSupport {
    ...
    public void testOrderToCsvBean() throws Exception {
        ...
        template.sendBodyAndHeader("direct:start", inhouse, "Date", "20091209");//// asta are loc prima data
       

        File file = new File("target/orders/received/report-20091209.csv");
        assertTrue("File should exist", file.exists());// aici ajung dupa ce s

        // compare the expected file content
        String body = context.getTypeConverter().convertTo(String.class, file);// aici fac rost de convertorul din camel si convertesc la strign ce am un file
        assertEquals("000000555,20091209,000001144,2319,1108", body);
    }

    @Override
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {// asta are loc a doua oara 
            @Override
            public void configure() throws Exception {
                from("direct:start")// avem ca body stringul house , headerul date cu valoarea 20091209
